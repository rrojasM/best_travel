...
public class SecurityConfig {
    ...
    /*CONFIGURACIÓN DE OAUTH2 PARA EL LOGIN EN LA APLICACION DESARROLLADA*/
    @Bean
    @Order(1)
    ...
    public SecurityFilterChain appSecurityFilterChain(HttpSecurity http) throws Exception{
        //DOS FORMAS DIFERENTES PARA AGREGAR REQUEST MATCHERS PARA LAS RUTAS EN LA AUTHENTICATION
        http
                .formLogin()
                .and()
                ...
                .requestMatchers(ADMIN_RESOURCES).hasAuthority(ADMIN_ROLE)
                .and()
        ...
        http
                ...
                .authorizeHttpRequests(
                        authorize -> {
                            authorize.requestMatchers(PUBLIC_RESOURCES).permitAll();
                        }
                )
                .authorizeHttpRequests(
                        authorize -> {
                            authorize.requestMatchers(USER_RESOURCES).authenticated();
                        }
                )
                .authorizeHttpRequests(
                        authorize -> {
                            authorize.requestMatchers(ADMIN_RESOURCES).hasAuthority(ADMIN_ROLE);
                        }
                )
                .oauth2ResourceServer()
                .jwt();
    ...
    private static final String ADMIN_ROLE = "write";
